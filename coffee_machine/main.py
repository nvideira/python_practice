...
while on:
    ...
    if order == "off":
        ...
        for src in data.resources:
            ...
            if data.resources[src] + add > data.max_capacity[src]:
                add = data.max_capacity[src] - data.resources[src]
                print(f"{src.capitalize()} capacity is now at max. Any excess {src} has been removed.")
